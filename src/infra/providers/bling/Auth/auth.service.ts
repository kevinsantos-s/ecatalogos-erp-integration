...
export async function authorizeBlingCompany(code: string, redirectUri: string) {
...
async function refreshToken(auth: any): Promise<string> {
  const body = new URLSearchParams();
  ...
  await saveBlingToken({
    blingCompanyId: auth.blingCompanyId,
    accessToken: access_token,
    refreshToken: refresh_token,
    expiresIn: expires_in,
  });
...
async function saveBlingToken({
  ...
  await prisma.blingAuth.upsert({
    ...
    create: {
      ...
      refreshToken,
      expiresAt,
      blingCompany: { connect: { blingCompanyId } },
    },
  });
}